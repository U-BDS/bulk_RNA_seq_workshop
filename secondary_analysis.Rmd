...
To begin, we'll use the interactive file system available through OnDemand Application provided by UAB Research Computing. The link can be found below: 

`https://rc.uab.edu/pun/sys/dashboard/`

We'll navigate to the directory where we have moved the files downloaded from Globus. Following the install instructions will have placed the files in the directory below (NOTE: The `<blazer_id>` in the path should be replaced with your blazer id):
...
> ** NOTE **
>
> For this lesson, we'll be using the onDemand Editor provided by UAB Research Computing in order to edit and create files. However, if you are more comfortable using a software editor such as `vim` or `nano`, that is also acceptable.
...
We’ve put the header into the samplesheet, so let’s save the samplesheet because we need to also include the FASTQs. In order to save the file, we click the `Save` button located in the upper right.
...
The above lines of code set the executor to be slurm, and set the queue so that all jobs will be submitted to the `amdhdr-100` partition on cheaha.
