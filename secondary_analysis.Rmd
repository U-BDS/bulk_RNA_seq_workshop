...
**The purpose of this workshop will be to provide a more in-depth look at secondary and tertiary analysis.**
...
The count outputs format typically contains a list of features as rows and their relative abundance as columns for each sample.
...
We will be using UAB's HPC cluster to demonstrate the nf-core/rnaseq pipeline, since most common use-cases require resources that may not be readily available in local computers. Thus as a first step, we will logging in the cluster.

Importantly, when implementing a novel pipeline or tool, it is critical to always test the pipeline to ensure that it is installed and working properly. Nf-core requires all pipelines to have downsampled data to be able to run a test profile.
...
Running the pipeline will yield a status update page. This page will show the current progress of the pipeline, completed jobs, failed jobs, etc. The command will also download the version of the pipeline specified so it can be run more easily in the future.
...
First, let's go to the directory where we downloaded the raw data using Globus (in the cheaha scratch space):
...
For the workshop, we’ll use vim. And we'll go ahead and enter the header
...
We’ve put the header into the samplesheet, so let’s save the samplesheet because we need to also include the FASTQs. The way we will do that is via the commandline.
...
```         
sample,fastq_1,fastq_2,strandedness
N01_AM_Naive,/data/project/U_BDS/Globus_endpoints/rnaseq_workshop/input/downsample_fastqs/SRX4328050_SRR7457559.fastq.gz,,forward
N02_AM_Naive,/data/project/U_BDS/Globus_endpoints/rnaseq_workshop/input/downsample_fastqs/SRX4328049_SRR7457560.fastq.gz,,forward
N03_AM_Naive,/data/project/U_BDS/Globus_endpoints/rnaseq_workshop/input/downsample_fastqs/SRX4328052_SRR7457557.fastq.gz,,forward
N04_AM_Naive,/data/project/U_BDS/Globus_endpoints/rnaseq_workshop/input/downsample_fastqs/SRX4328051_SRR7457558.fastq.gz,,forward
R05_AM_Allo24h,/data/project/U_BDS/Globus_endpoints/rnaseq_workshop/input/downsample_fastqs/SRX4328058_SRR7457551.fastq.gz,,forward
R06_AM_Allo24h,/data/project/U_BDS/Globus_endpoints/rnaseq_workshop/input/downsample_fastqs/SRX4328057_SRR7457552.fastq.gz,,forward
R07_AM_Allo24h,/data/project/U_BDS/Globus_endpoints/rnaseq_workshop/input/downsample_fastqs/SRX4328048_SRR7457561.fastq.gz,,forward
R08_AM_Allo24h,/data/project/U_BDS/Globus_endpoints/rnaseq_workshop/input/downsample_fastqs/SRX4328047_SRR7457562.fastq.gz,,forward
```
...
> **Discussion Question**
>
> You'll note that we have two commas next to each other in the samplesheet, which means we have left a column empty. The data for this workshop is single-ended, meaning that there is just a single fastq for us to analyze. Its also common for data to be paired end, which for us would result in a pair of fastqs for each sample, known as an read 1 and read 2.
>
> Can anyone tell me what we would change with this samplesheet if the data was paired end instead of single end?
>
> **Solution** We would need to fill in the column that is left blank. The samplesheet headers help guide us in what information needs to be placed in each column. In this case, `fastq_1` would correspond to the read 1 fastq and `fastq_2` would correspond to the read 2 fastq.
...
-   `deseq2_vst` will use the vst transformation over deseq's rlog. This is ideal as vst is faster to run and more suitable for larger datsets
...
Lets start by changing the queue in a file called `custom.conf`. The `cheaha` profile will pick a queue based on the profile defaults. Since we’re using donwsampled data, everything will run very quickly, in order to facilitate that we’ll make it so the default queue is 'express':
...
The above lines of code set the executor to be slurm, and set the queue so that all jobs will be submitted to the `express` partition on cheaha
...
We can see that the salmon module is located at `modules/nf-core/salmon/quant`. Let's open up the process file:
...
With the name of the process, let’s go back and open up the custom configuration file and add the below lines.
...
With all the files created, we can submit the pipeline using the below command:
...
> ``` samplesheet.csv
> sample,fastq_1,fastq_2,strandedness
> N01_AM_Naive,/data/project/U_BDS/Globus_endpoints/rnaseq_workshop/input/fastqs/SRX4328050_SRR7457559.fastq.gz,,forward
> N02_AM_Naive,/data/project/U_BDS/Globus_endpoints/rnaseq_workshop/input/fastqs/SRX4328049_SRR7457560.fastq.gz,,forward
> N03_AM_Naive,/data/project/U_BDS/Globus_endpoints/rnaseq_workshop/input/fastqs/SRX4328052_SRR7457557.fastq.gz,,forward
> N04_AM_Naive,/data/project/U_BDS/Globus_endpoints/rnaseq_workshop/input/fastqs/SRX4328051_SRR7457558.fastq.gz,,forward
> R05_AM_Allo24h,/data/project/U_BDS/Globus_endpoints/rnaseq_workshop/input/fastqs/SRX4328058_SRR7457551.fastq.gz,,forward
> R06_AM_Allo24h,/data/project/U_BDS/Globus_endpoints/rnaseq_workshop/input/fastqs/SRX4328057_SRR7457552.fastq.gz,,forward
> R07_AM_Allo24h,/data/project/U_BDS/Globus_endpoints/rnaseq_workshop/input/fastqs/SRX4328048_SRR7457561.fastq.gz,,forward
> R08_AM_Allo24h,/data/project/U_BDS/Globus_endpoints/rnaseq_workshop/input/fastqs/SRX4328047_SRR7457562.fastq.gz,,forward
> ```
>
