...
Let's go ahead and peek at the FASTQ file itself. We'll use `gunzip -c` to produce the file contents to stdout (display output), and we'll just look at the first 10 lines using `head` command.
...
|   Q-Score |   ASCII Code | Symbol   |   Q-Score |   ASCII Code | Symbol   |   Q-Score |   ASCII Code | Symbol   |
|-----------|--------------|----------|-----------|--------------|----------|-----------|--------------|----------|
|         0 |           33 | !        |        14 |           47 | /        |        28 |           61 | =        |
|         1 |           34 | "        |        15 |           48 | 0        |        29 |           62 | >        |
|         2 |           35 | #        |        16 |           49 | 1        |        30 |           63 | ?        |
|         3 |           36 | $        |        17 |           50 | 2        |        31 |           64 | @        |
|         4 |           37 | %        |        18 |           51 | 3        |        32 |           65 | A        |
|         5 |           38 | &        |        19 |           52 | 4        |        33 |           66 | B        |
|         6 |           39 | '        |        20 |           53 | 5        |        34 |           67 | C        |
|         7 |           40 | (        |        21 |           54 | 6        |        35 |           68 | D        |
|         8 |           41 | )        |        22 |           55 | 7        |        36 |           69 | E        |
|         9 |           42 | *        |        23 |           56 | 8        |        37 |           70 | F        |
|        10 |           43 | +        |        24 |           57 | 9        |        38 |           71 | G        |
|        11 |           44 | ,        |        25 |           58 | :        |        39 |           72 | H        |
|        12 |           45 | -        |        26 |           59 | ;        |        40 |           73 | I        |
|        13 |           46 | .        |        27 |           60 | <        |        41 |           74 | J        |
...
-   `lib` - Contains groovy libraries for the pipeline to use
...
> ```
>
> `run_rnaseq.sh` maintains the same time limit as our downsample data. For most RNA-seq analysis, this is sufficient, but if your pipeline fails due to time limit, simply increase this further.
> 
> ``` run_rnaseq.sh
> #!/usr/bin/env bash
> 
...
> **Discussion Question**
>
> In the runs above, we wrote a custom configuration for the Salmon flags as an demostration of how to pass in tool-specific parameters for nf-core pipelines.
> This was a good example on how to configure your pipeline in a more advanced manner (e.g.: when no alternative approach exist).
>
> However, the pipeline authors have also provided explicit parameters in this pipeline which enables users to accomplish the same task in a more straight-forward manner.
> From looking at the documentation, what is the alternative approach of providing the `--gcBias --seqBias` parameters to Salmon?
>
> <details>
> <summary>**Click here for solution**</summary>
> You can add the following to your `params.yml` with the pipeline flag `extra_salmon_quant_args`
>
>```
> extra_salmon_quant_args: "--seqBias --gcBias"
>```
>
> **NOTE**: this option only became available with version >= 3.10. This is a good example of the advantages of always checking the pipeline release notes for updates/improvements and bug fixes.
>
> </details>
...
Lastly, the various `work` directories are also useful for troubleshooting the pipeline as they contain other "hidden"

files (files starting with a `.`) and logs that are useful for the the troubleshooting process.
