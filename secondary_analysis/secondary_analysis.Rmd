...
-   **Secondary Analysis** - Secondary analysis are the steps in a
    ...
    For bulk RNA-seq analyses, the quantification represents expression count files.
...
uploaded and downloaded from other sources, so the md5 can be computed to determine
...
does not provide any md5’s, it is worth reaching out to the source with this request.
Once these are generated you are able to check the md5s of the
...
approach to alignment, the and nf-core/rnaseq pipelines supports two methods.
...
The second type of alignment/mapping supported is "quasi-mapping". In standard
<!-- TODO: add reference for further reading on alingment methods/stragegies -->
...
-   `-profile test, cheaha` : Profiles can be thought of as presets for
    ...
    for the pipeline. The ‘cheaha’ profile is what called an
    institutional profile, the one for use at UAB is ‘cheaha’, but there
    are also other institutions and you are able to create your own if
    one doesn’t already exist. The one thing to note is that if you pass
    in multiple profiles, the last one takes precedence.
...
doing your own analysis to see if there are any flags that may apply, as
...
the pipeline to produce pseudo aligments with Salmon. There's no good
rule of thumb about when to use STAR mappings over Salmon mappings, as
it is highly depending on the design of the experiment. For example, if
you also have genome data being processed you may want to use the STAR
results as its easier to map the alignments back due to Salmon not
producing a bam file. We typically execute the pipeline so that both
types of aligners are run (as demonstrated above), that way the
...
salmon process, we don’t want to run it for everything. Since `withName`
...
The rnaseq pipeline produces a lot of output files. A lot of the output
is for qc and general review of the pipeline user. Due to the amount of
...
The `salmon` results are produced by performing pseudoalignment using
Salmon. This method will NOT produce any alignment files. This is the
case of all pseudoalignment tools as such tools do not produce
alignments, only statistical count files.
