...
-   **Primary Analysis** - There can be a bit of a disjoint in the
    ...
    sequencing lab and the end result of this will be a demultiplexed
    FASTQ. That is very much an oversimplification about what happens at
    this step, as there is a lot of complexity, and entire workshops
    could be given (and are given) solely on this step.

-   **Secondary Analysis** - Secondary analysis is the steps in a
    workflow where FASTQ files are turned into the files you need for
    tertiary. The files produced differ depending on what kind of
    analysis you are performing, a single cell analysis would need a
    feature-barcode matrix, whole genome analysis would work with
    variant call files. For bulk rna analyses we are concerned with
...
can tell its a FASTQ because it has the ‘.fq’ file extension, but you’ll
...
the file compression algorithm. The reason we compress files is to
minimize the storage space the file takes up on a file system, and this
is an especially important for FASTQs because they can be very large
files.

Lets go ahead and peek at the FASTQ file itself. We'll use `gunzip -c`
...
3.  A **separator**. Starts with a '+' sign. Sometimes this will also
    ...
    identifier, but its also not uncommon for this to only contain the
    '+' sign.
...
> **Discussion Question**
>
> Has anyone performed any rnaseq analysis? What tools have you used?
...
allows for a degree of configuration to give the user the choice of
which tools to run.
