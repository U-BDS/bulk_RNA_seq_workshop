...
###### salmon, or STAR-salmon

Locate all the transcript abundance file.
...
txi <- tximport(myFiles_names, type = "salmon", tx2gene = tx2gene, txOut = FALSE, ignoreTxVersion=FALSE)
names(txi)
head(txi$counts, 5)
...
keep <- rowMeans(counts(dds)) >= 5 # here we do rowMeans, other approaches are rowSums or min. per sample
dds <- dds[keep,]
...
Principal component (PC) analysis plot displaying all 12 samples along PC1 and PC2, which describe 66% and 19% of the variability, respectively, within the expression data set.
