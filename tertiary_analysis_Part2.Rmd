...
top_up_genes <- Transplant_vs_Naive_annotated_DEGs[order(Transplant_vs_Naive_annotated_DEGs$padj), ] %>%
  filter(log2FoldChange > 1) %>%           # Focus on up-regulated (positive log2fc)
  head(20) %>%
  rownames()

# Select top 10 down-regulated genes
top_down_genes <- Transplant_vs_Naive_annotated_DEGs[order(Transplant_vs_Naive_annotated_DEGs$padj), ] %>%
...
```{r}
Transplant_vs_Naive_annotated_fil <- Transplant_vs_Naive_annotated[! is.na(normalized_counts_annotated$external_gene_name)
                                                                   & Transplant_vs_Naive_annotated$external_gene_name !="", ]

normalized_counts_annotated_fil <- normalized_counts_annotated[! is.na(normalized_counts_annotated$external_gene_name)
                                                               & normalized_counts_annotated$external_gene_name !="", ]

unique_gene_names <- make.unique(Transplant_vs_Naive_annotated_fil$external_gene_name, sep = ".")


rownames(Transplant_vs_Naive_annotated_fil) <- unique_gene_names

rownames(normalized_counts_annotated_fil) <- unique_gene_names
```

```{r}

# Get top 10 DE genes
genes <- Transplant_vs_Naive_annotated_fil[order(Transplant_vs_Naive_annotated_fil$padj), ] %>%
         head(10) %>%
         rownames()

heatmapData <- normalized_counts_annotated_fil[genes, ]
...
goi <- Transplant_vs_Naive_annotated_fil[order(Transplant_vs_Naive_annotated_fil$padj), ] %>%
         head(10) %>%
         rownames()
```
...
countData <- normalized_counts_annotated_fil[goi, 1:8]
